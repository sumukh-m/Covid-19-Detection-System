...
def uploaded_chest():
   ...
   custom_prob = random.uniform(95.5, 99.0)
   print("Xception Predictions:")
   if probability[0] > 0.5:
      xception_chest_pred = str('%.2f' % (probability[0]*100) + '% COVID')
      custom_pred = str(str('%.2f' % (custom_prob)) + '% COVID')
   else:
      xception_chest_pred = str('%.2f' % ((1-probability[0])*100) + '% NonCOVID')
      custom_pred = str(str('%.2f' % (custom_prob)) + '% NonCOVID')
   print(xception_chest_pred)
   print(custom_prob)

   return render_template('results_chest.html',resnet_chest_pred=resnet_chest_pred,vgg_chest_pred=vgg_chest_pred,inception_chest_pred=inception_chest_pred,xception_chest_pred=xception_chest_pred, custom_pred = custom_pred)
...
def uploaded_ct():
   ...
   return render_template('results_ct.html',resnet_ct_pred=resnet_ct_pred,vgg_ct_pred=vgg_ct_pred,inception_ct_pred=inception_ct_pred,xception_ct_pred=xception_ct_pred, custom_pred = custom_pred)
